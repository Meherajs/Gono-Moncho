...
contract ReporterRegistryTest is Test {
    ...
    function testStakeTokens() public {
        ...
        (,, uint256 stakedAmount,,,,, ) = registry.getReporterProfile(reporter);
    ...
    function testVerifyReporter() public {
        ...
        (, ReporterRegistry.ReporterStatus status,,,,,, ) = registry.getReporterProfile(reporter);
    ...
    function testRegisterAnalyzer() public {
        vm.prank(analyzer);
        registry.registerReporter("QmAnalyzer", ReporterRegistry.UserRole.ANALYZER);
        
        (ReporterRegistry.UserRole role,,,,,,, ) = registry.getReporterProfile(analyzer);
        assertEq(uint(role), uint(ReporterRegistry.UserRole.ANALYZER));
    }

    function testRegisterVerifier() public {
        vm.prank(verifier);
        registry.registerReporter("QmVerifier", ReporterRegistry.UserRole.VERIFIER);
        
        (ReporterRegistry.UserRole role,,,,,,, ) = registry.getReporterProfile(verifier);
    ...
    function testReporterCannotVerify() public {
        ...
        vm.prank(reporter);
        registry.registerReporter("QmReporter", ReporterRegistry.UserRole.REPORTER);
